...
*/
module.exports = class dominknow_test_questions_responses_fillin extends AnalyticProcessor {
    constructor(params, db, lrs) {
        ...
        this.pipeline = [
            ...
            {// testMode
                $match: {
                    'statement.object.definition.interactionType': 'fill-in',
    ...
    exec(data) {
        ...
        for (const i in data) {
            res.push({
                icon: 'fa-edit',
                title: 'Response:' + data[i]._id,
                subtext: 'Submitted ' + data[i].count + ' times.',
            });
        }
        return res;
    }
